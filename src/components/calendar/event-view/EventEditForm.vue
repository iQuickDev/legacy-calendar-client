...
const onSave = () => {
    ...
    const end = endDateOnly.value && endTimeOnly.value
        ? combineDateAndTime(endDateOnly.value, endTimeOnly.value)
        : undefined;
...
<template>
    <div class="flex flex-col gap-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <Textarea id="edit-desc" v-model="description" rows="4" placeholder="Add a description..." class="rounded-xl!" />
                </div>

                <div class="flex flex-col gap-2">
                    <label for="edit-location" class="font-bold text-sm uppercase tracking-wider text-zinc-500">Location</label>
                    <InputText id="edit-location" v-model="location" placeholder="Meeting Room, Online, etc." class="rounded-xl!" />
                </div>
                ...
                <div class="flex flex-col gap-2 mt-2">
                    ...
                    <MultiSelect id="edit-participants" v-model="selectedParticipants" :options="users"
            ...
            <div class="flex flex-col gap-4">
                ...
                <div class="space-y-3">
                    <div v-for="feature in FEATURES" :key="feature.id"
                        ...
                        :class="[selectedFeatures.includes(feature.id) ? 'bg-zinc-50 dark:bg-zinc-900/50 border-emerald-500/30' : 'border-zinc-200 dark:border-zinc-800']">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-xl flex items-center justify-center text-xl"
                                    :class="selectedFeatures.includes(feature.id) ? feature.color : 'bg-zinc-100 dark:bg-zinc-800 opacity-50'">
                                    {{ feature.icon }}
                                </div>
                                <span class="font-bold" :class="selectedFeatures.includes(feature.id) ? '' : 'text-zinc-500'">{{ feature.label }}</span>
                            </div>
        ...
        <div class="flex justify-end w-full gap-2 pt-2">
            <Button label="Cancel" icon="pi pi-times" severity="secondary" text @click="emit('cancel')" :disabled="saving" />
